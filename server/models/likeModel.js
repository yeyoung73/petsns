...
export const addLike = async (userId, postId) => {
  const query = `
    INSERT INTO petsns.likes (user_id, post_id)
    VALUES ($1, $2)
...
export const removeLike = async (userId, postId) => {
  const query = `
    DELETE FROM petsns.likes WHERE user_id = $1 AND post_id = $2
  `;
  await pool.query(query, [userId, postId]);
};
...
export const countLikes = async (postId) => {
  const query = `SELECT COUNT(*) FROM petsns.likes WHERE post_id = $1`;
...
export const hasUserLiked = async (userId, postId) => {
  const query = `SELECT 1 FROM petsns.likes WHERE user_id = $1 AND post_id = $2`;
